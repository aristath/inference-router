...
impl VRAMTracker {
    ...
    /// Same as `refresh` but with an injectable sysfs root.
    pub fn refresh_from(&self, sysfs_root: &str) -> Vec<GpuInfo> {
        self.refresh_from_with_vulkan(sysfs_root, vulkan_devices_by_pci())
    }

    /// Core of `refresh_from` with the Vulkan device map injected, which keeps
    /// device classification independent of live `vulkaninfo` output.
...
fn parse_rocm_showbus(text: &str) -> HashMap<String, usize> {
    ...
    for line in text.lines() {
        let Some(open) = line.find("GPU[") else {
            continue;
        };
